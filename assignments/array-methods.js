...
ticketPriceTotal = runners.reduce(function (ticketPriceTotal, currentValue) {
...
// Problem 1
// Populate an Array with all of the company names so that a banner can be created with a list of sponsors. 

let companies = [];

runners.map(function (el) {
  companies.push(`${el.company_name}`);
});
console.log(companies);


// Problem 2
// Create an array with all the info of the top donors (over $200) so the organization can save their information

let topDonors = [];

topDonors = runners.filter(function (el) {
  return el.donation > 200
});
...
// Problem 3
// Create a new array with just the emails so that they can create a mailing list with updates for the event and future events
